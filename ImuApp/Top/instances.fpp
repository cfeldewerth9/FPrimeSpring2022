module IMUApp {
  ...
  instance IMU: ImuApp.Imu base id 0x0F00 \
    queue size Default.queueSize \
    stack size Default.stackSize \
    priority 80
  ...
  instance IMU_I2C: Drv.LinuxI2cDriver base id 0x4C00 \
  {
    phase Fpp.ToCpp.Phases.configComponents  """
    {
    const bool status = IMU_I2C.open(state.device);
    
    if (!status) {
      Fw::Logger::logMsg("[ERROR] Could not open IMU I2C driver: %s\\n", reinterpret_cast<POINTER_CAST>(state.device));
      Init::status = false;
      }
    else {
      Fw::Logger::logMsg("[INFO] Opened IMU I2C driver: %s\\n", reinterpret_cast<POINTER_CAST>(state.device));
      Init::status = true;
    }
    }
    """
